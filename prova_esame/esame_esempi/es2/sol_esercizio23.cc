...
void get_elements_aux(const int source[],
  ...
  if (i >= size_s) { // caso base
    ...
    if ((i % 2 != 0) && (source[i] % 2 == 0)) {
      int p = d1++; // Salvo la posizione dove inserire il numero
      ...
      dest1[p] = (source[i] + 10)/3;
    } else if ((i % 2 == 0) && (source[i] % 5 == 0)) {
      int p = d2++; // Salvo la posizione dove inserire il numero
